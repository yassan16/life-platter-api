...
- **なぜサーバーに保存しないか**: JWTは自己完結型で署名検証のみで認証可能（DB問い合わせ不要・高速）
...
- **なぜサーバーに保存するか**: 7日間有効なため漏洩時に即座に無効化できる仕組みが必要
...
- サーバーは受け取ったトークンの Header + Payload を秘密鍵で再署名し、Signature と比較
- 一致すれば認証OK、不一致なら改ざん検知
- DB 問い合わせ不要でメモリ内で完結（高速）

### リフレッシュトークンローテーション

リフレッシュ時には**両方のトークンが新規発行**される。古いリフレッシュトークンは無効化されるため、漏洩時の被害を限定できる。また、盗まれたトークンで二重使用されると「既に使用済み」エラーで不正アクセスを検知できる。
